...
async def Prosmotr(message: Message, state: FSMContext) -> None:
    await state.clear()
...
async def command_list_currencies_handler(message: Message, state: FSMContext) -> None:
    await state.clear()
    await message.answer(""" 
...
async def get_moneyC(message: Message, state: FSMContext):
    await state.clear()
    response = requests.get(KursRUB)
    t = response.json()
